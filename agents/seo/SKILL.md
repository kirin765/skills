---
name: seo
description: "Comprehensive SEO analysis for any site: audits (full or single-page), technical SEO (crawlability, indexability, Core Web Vitals/INP), schema markup, content quality (E-E-A-T), images, sitemaps, GEO for AI Overviews/ChatGPT/Perplexity, industry detection. Triggers on: SEO, audit, schema, Core Web Vitals, sitemap, E-E-A-T, GEO, page speed, structured data."
user-invokable: true
...
8. **seo-geo** -- AI Overviews / GEO optimization. Mechanism (from ChatGPT network teardown): AI reads *your page for facts, others' pages for opinion*. Top levers — facts (pricing/specs) in **crawlable plain HTML, not JS** (a JS pricing table hands your numbers to G2); **one strong page per claim** (results dedupe by domain); earn citations **off-site** (you can't cite yourself); text > video; target **fetch-triggering** queries (training-answered how-tos can't be won).
