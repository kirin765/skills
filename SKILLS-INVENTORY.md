...
| 스킬 | 용도 |
|---|---|
| `loop-engineering` | 반복작업을 자기-프롬프트 에이전트 루프로 설계하는 대화형 코치. 14단계 "prompter→loop designer" 로드맵을 자격심사(4조건 테스트·30초 체크)부터 게이트로 진행 — 대부분은 "아직 불필요"로 정직하게 종료, 통과 시 최소 루프(자동화·스킬·상태파일·게이트) 4요소를 Claude Code 아티팩트로 조립하고 실패모드(Ralph Wiggum 조용한 실패·이해부채·보안세) 방어까지. 빌트인 `/loop`(단순 반복 실행)와 구분 — 이건 루프를 *지을지 말지*와 구조를 설계. |
