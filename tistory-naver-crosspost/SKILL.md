...
- Category selectbox (publish panel, 2026-07-26 live-verified): trigger is `button[class*='selectbox_button']` (aria-label "카테고리 목록 버튼"), items are `span[data-testid^='categoryItemText_']` inside `label.radio_label__*`. Sub-category items prepend a blind "하위 카테고리" span — strip it before exact-matching the name. **JS `el.click()` does NOT register with React** (the publish silently goes out with the previous category even though the click "succeeds") — use a real Playwright click, then verify the trigger button's text changed to the target before publishing.
- Final publish button (publish panel): `[data-testid='seOnePublishBtn']`, fallback `button[class*='confirm_btn']`, last resort the last visible button whose exact text is `발행`.
