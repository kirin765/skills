---
name: tistory-naver-crosspost-v2
description: |
  v2 update: Cross-post a blog article from a source (a live URL or local HTML file) to Tistory and Naver Blog via Chrome CDP (port 9222). Includes Kakao DKAPTCHA auto-solver automation and enhanced Tistory publish modal logic. Trigger on "티스토리/네이버 v2", "tistory-naver-crosspost-v2", "티스토리 네이버 크로스포스트 v2".
---

# Tistory + Naver Blog Crosspost v2 (CDP)

## What this does (v2 Update)

Given a source article (live URL **or** local HTML file), hero image, title, and tag list, this skill (v2) drives the user's existing Chrome session (CDP port 9222) to:

> **v2 Enhancements:**
> - **Kakao DKAPTCHA auto-solver (`solveTistoryDkaptcha`)**: Detects Kakao DKAPTCHA popup iframe (`iframe[src*='dkaptcha']`), automatically inputs answer text, dispatches synthetic React input/change/keyup events to enable the submit button, and submits the answer.
> - **Tistory Publish Automation**: Supports fully automated draft publishing and Kakao DKAPTCHA bypass handling.
