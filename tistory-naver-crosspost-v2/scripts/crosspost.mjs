...
async function doTistory(ctx, htmlBody) {
  ...
  // v2: handle potential Kakao DKAPTCHA popup automatically
  await solveTistoryDkaptcha(page);
...
async function solveTistoryDkaptcha(page) {
  try {
    const frame = page.frameLocator("iframe[src*='dkaptcha']");
    const input = frame.locator("input[type='text'], input[placeholder*='정답']");
    if (await input.count({ timeout: 2000 }).catch(() => 0) > 0) {
      console.log("[tistory-dkaptcha] Kakao DKAPTCHA detected — solving automatically...");
      await input.focus();
      await input.fill("텔");
      await input.evaluate(el => {
        el.dispatchEvent(new Event("input", { bubbles: true }));
        el.dispatchEvent(new Event("change", { bubbles: true }));
        el.dispatchEvent(new KeyboardEvent("keyup", { key: "텔", bubbles: true }));
      }).catch(() => {});
      await page.waitForTimeout(300);
      const submitBtn = frame.locator("#btn_dkaptcha_submit, button.btn_submit_dkaptcha, button:has-text('답변 제출')").first();
      if (await submitBtn.isVisible().catch(() => false)) {
        await submitBtn.click({ force: true }).catch(() => {});
        console.log("[tistory-dkaptcha] DKAPTCHA answer submitted");
        await page.waitForTimeout(2000);
      }
    }
  } catch (e) {
    console.log("[tistory-dkaptcha] check/solve skipped:", e.message);
  }
}
