---
name: coupang-api
description: Call the Coupang Open API (쿠팡 오픈 API / Coupang Wing) for the user's OWN store "온누리문방구" (vendorId A01569984) using the registered Access/Secret Key. Use this skill whenever the user wants to read or change anything in their Coupang store via API — create/register a product (상품 생성·등록, 로켓그로스 상품 생성 포함), list/search/edit products(상품 목록·조회·수정), change price or stock(가격·재고 수정), query orders(주문 조회·발주서·ordersheets), process shipping/invoices(배송·송장 등록), handle returns/exchanges(반품·교환), look up shipping/return centers(출고지·반품지), or pull settlement/revenue(정산·매출) — even if they don't name the API. Trigger on phrases like "쿠팡 API", "쿠팡 상품 등록/조회/수정", "온누리문방구 주문 가져와", "로켓그로스 상품 생성", "쿠팡 재고/가격 바꿔줘", "쿠팡 발주서/송장", "쿠팡 정산 내역", "Coupang Open API", "Coupang Wing API". This handles the tricky per-request HMAC-SHA256 (CEA) signing for you via a bundled script. It is for the user's own store through the official Open API — distinct from naver-commerce-api (their SmartStore). IMPORTANT GOTCHA: Coupang enforces an IP allowlist — calls only work from an IP registered in Wing 연동정보 (see "IP 허용목록" below), otherwise every call returns 403 FORBIDDEN regardless of correct signing.
---
