...
- **내부링크 (2026-07-27 추가)**: 1번에서 읽은 RSS의 기존 글 중 주제가 실제로 이어지는 글 1~3개를 골라, FAQ 섹션 뒤(글 맨 끝)에 붙인다. 형식은 아래처럼 안내 문단 + URL을 각각 별도 `<p>`로 — 발행기가 URL 줄을 타이핑하면 네이버가 링크 카드로 변환한다. 무관한 글을 억지로 넣지 않는다(관련 글이 없으면 0개로 생략). 링크 대상은 반드시 RSS에 실존하는 글만.
  ```html
  <p>📌 함께 읽으면 좋은 글</p>
