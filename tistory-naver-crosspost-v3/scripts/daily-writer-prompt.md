...
- **내부링크 (2026-07-27 추가 · 2026-08-12 정정)**: 1번에서 읽은 RSS의 기존 글 중 주제가 실제로 이어지는 글 1~3개를 골라, FAQ 섹션 뒤(글 맨 끝)에 붙인다. 형식은 아래처럼 안내 문단 + URL을 각각 별도 `<p>`로 — 발행기가 양쪽 블로그(네이버·티스토리)에서 URL 줄을 **클릭 가능한 링크**로 만들어 게시한다(타이핑 자동 변환은 불확실해 파이프라인이 명시적으로 링크를 삽입한다). 무관한 글을 억지로 넣지 않는다(관련 글이 없으면 0개로 생략). 링크 대상은 반드시 RSS에 실존하는 글만.
  ```html
  <p>📌 함께 읽으면 좋은 글</p>
  <p>https://blog.naver.com/kwan765/224350386577</p>
  ```
