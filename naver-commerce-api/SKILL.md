...
description: Call the Naver Commerce (스마트스토어/SmartStore) API for the user's OWN store "온누리문방구" (type=SELF credentials). Covers every store read/write — 상품 목록/검색/조회/수정/등록, 가격·재고, 주문 조회(신규/변경 포함), 발송/취소/반품 처리, 정산. Trigger on "스마트스토어 API", "내 스토어 상품/주문/재고/가격 …", "온누리문방구 주문 가져와", "커머스API", "Naver Commerce API" — even if the API isn't named. A bundled script handles bcrypt 전자서명 auth + token caching. Distinct from naver-api (search trends), naver-searchad (ads), naver-seo (on-page SEO). Ungranted API groups return GW.AUTHN — enable them in 커머스API센터 first.
---
